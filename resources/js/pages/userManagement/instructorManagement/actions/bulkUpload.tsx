...
export default function BulkUpload({ templateUrl }: PageProps) {
    ...
    async function handlePreview(e: React.FormEvent) {
        ...
        try {
            // <- backend expects /user-management/instructor/bulk/upload
            const { data } = await axios.post('/user-management/instructor/bulk/upload', form, {
    ...
    async function handleCommit() {
        ...
        const { isConfirmed } = await MySwal.fire({
            title: `Insert ${toInsert.length} instructors?`,
            text: 'This will create user and instructor records for each selected row.',
            icon: 'question',
        ...
        setCommitting(true);
        try {
            const { data } = await axios.post('/user-management/instructor/bulk/insert', { rows: toInsert });
            ...
            // Redirect back to instructors list (use simple navigation so we don't require @inertiajs/inertia)
            window.location.href = '/user-management/instructor';
        } catch (err: any) {
    ...
    return (
        <AppLayout
            breadcrumbs={[
                { title: 'Instructor Management', href: '/user-management/instructor' },
                { title: 'Bulk Insert', href: '/user-management/instructor/bulk' },
            ]}
        >
            <Head title="Bulk Insert — Instructor Management" />
                ...
                <div className="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
                    ...
                    <div className="mb-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-gray-100">Bulk Insert Instructors</h1>
                        ...
                        <Button asChild variant="outline" className="border-gray-200 dark:border-gray-700">
                            <Link href="/user-management/instructor" className="flex items-center gap-2">
                    ...
                    <div className="mb-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
                            <div className="space-y-1">
                                ...
                                <a
                                    href={templateUrl ?? '/templates/InstructorManagementBulkInsertTemplate.xlsx'}
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow className="border-b-2 border-gray-200 bg-gray-50 dark:border-gray-600 dark:bg-gray-700">
                                            ...
                                            <TableHead>Instructor Type</TableHead>
                                        ...
                                        {rows.map((r, i) => {
                                            ...
                                            return (
                                                <TableRow key={i} className={hasErrors ? 'bg-red-50/50 dark:bg-red-950/10' : ''}>
                                                    ...
                                                    <TableCell className="font-mono">{r.instructor_code ?? '—'}</TableCell>
                                                    <TableCell className="font-medium">{displayName(r) || '—'}</TableCell>
                                                    <TableCell>{r.instructor_type ?? '—'}</TableCell>
                                                    ...
                                                    <TableCell>{r.date_started ?? '—'}</TableCell>
                                                    <TableCell className="text-sm">
                                                        {r.valid ? (
