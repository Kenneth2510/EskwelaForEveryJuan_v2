...
import { ChevronLeft, ChevronRight, Filter, Search, Upload, UserPlus, Users } from 'lucide-react';
import * as React from 'react';
import ExportInstructors from './actions/export';
...
export function DataTable<TData, TValue>({ columns, data }: DataTableProps<TData, TValue>) {
    ...
    const table = useReactTable({
        ...
        globalFilterFn: (row, columnIds, filterValue) => {
            const searchableCols = ['instructor_code', 'name', 'instructor_type', 'email', 'status'];
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">Instructor Directory</h2>
                            <p className="flex items-center gap-1 text-sm text-gray-600 dark:text-gray-400">
                                <span className="inline-block h-2 w-2 animate-pulse rounded-full bg-green-500"></span>
                                {data.length} instructors registered
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        >
                            <Link href="/user-management/instructor/create" className="flex items-center gap-2 px-6 py-2">
                                <UserPlus className="h-4 w-4" />
                                <span className="hidden sm:inline">Add New Instructor</span>
                        ...
                        >
                            <Link href="/user-management/instructor/bulk" className="flex items-center gap-2 px-4 py-2">
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <div className="flex flex-col items-stretch gap-4 sm:flex-row sm:items-center">
                    <div className="relative max-w-md flex-1">
                        ...
                        <Input
                            placeholder="Search instructor by name, email, instructor type, status..."
                            value={globalFilter ?? ''}
                                ...
                                <TableRow>
                                    <TableCell colSpan={columns.length} className="h-32 text-center">
                                        <div className="flex flex-col items-center gap-3 text-gray-500 dark:text-gray-400">
                                            ...
                                            <div>
                                                <p className="font-medium">No instructor found</p>
                ...
                {table.getPageCount() > 1 && (
                    <div className="flex flex-col items-center justify-between gap-4 border-t border-gray-200 bg-gray-50 p-4 sm:flex-row dark:border-gray-600 dark:bg-gray-700">
                        <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
                            ...
                            <span>{table.getFilteredRowModel().rows.length} total instructors</span>
                        </div>
