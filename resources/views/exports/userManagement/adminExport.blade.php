...
<head>
    <meta charset="UTF-8" />
    <title>Admin Directory Report</title>
    <style>
        /* Basic reset */
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: Arial, Helvetica, sans-serif; }
        ...
        .admin-code { color: #991B1B; font-weight: 600; font-size: 11px; }
...
<body>
    @php
        // Counts
        $total = is_countable($admins) ? count($admins) : 0;
    ...
    <header class="report-header">
        ...
        <div class="summary" style="margin-top:12px;">
            <div class="card" style="width: 32%;">
                ...
                <div class="kpi-label">Total Admin</div>
            </div>
    ...
    <main>
        @if($total > 0)
            <div class="table-wrap">
                <table class="report-table" cellpadding="0" cellspacing="0" role="table" aria-label="Students">
                    <thead>
                        <tr>
                            <th style="width:14%;">Admin code</th>
                    ...
                    <tbody>
                        @foreach($admins as $admin)
                            <tr>
                                <td class="admin-code">{{ $admin->admin_code ?? 'N/A' }}</td>

                                <td class="admin-name">
                                    @if(optional($admin->user))
                                        {{ trim((optional($admin->user)->fname ?? '') . ' ' . (optional($admin->user)->mname ? optional($admin->user)->mname . ' ' : '') . (optional($admin->user)->lname ?? '')) }}
                                    @else
                                        N/A
                                    @endif
                                </td>
                                ...
                                <td>
                                    <div class="meta">{{ optional($admin->user)->email ?? 'N/A' }}</div>
                                    <div class="meta">{{ optional($admin->user)->phone ?? 'N/A' }}</div>
                                </td>

                                <td>
                                    @php $st = strtolower(optional($admin->user)->status ?? 'inactive'); @endphp
        ...
        @else
            <div style="padding:28px; text-align:center; color:#6B7280;">
                ...
                <h3 style="margin-bottom:6px;">No admins found</h3>
                <p>No admins match the current criteria.</p>
            </div>
        @endif
    </main>
    ...
    <footer class="report-footer">
        <span class="pagenum"></span> · Admin Management System · © {{ date('Y') }}
    </footer>
</body>
</html>
